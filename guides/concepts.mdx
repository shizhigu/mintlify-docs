---
title: Core Concepts
description: Understand the Moss data model — workspaces, contacts, companies, deals, and more.
---
...
| `demo` | Discovery or demo scheduled/completed |
...
Tags provide flexible categorization across contacts, companies, and deals. Moss uses a **polymorphic tagging** system — a single tag can be applied to any entity type via a join table.
...
Moss supports two authentication methods:
...
Moss provides full-text search powered by PostgreSQL `tsvector` with GIN indexes. Search across contacts, companies, deals, and interactions with a single query:
